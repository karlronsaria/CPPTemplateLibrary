...
typedef Complex<flt_t> complex_t;
...
complex_t pow(const complex_t& z, int_t a);
complex_t pow(flt_t a, const complex_t& z);
complex_t pow(const complex_t& z, const complex_t& w);
complex_t fpow(const complex_t& z, flt_t w);
...
template <typename T>
const Complex<T> operator/(const T& scalar, const Complex<T>& vector) {
    return complex_t{ scalar, 0 } / vector;
}
