...
namespace skiplist {
	...
	private:
		...
		void init() {
			_head = new Node(Bottom, SortedSet::MAX_LEVELS);
			_size = 0;
			_levels = SortedSet::INIT_LEVELS;
			_singleton();
		}

		static size_t next_random(size_t levels) {
			size_t level = 0;
			size_t r = rand();
		...
		static bool push(Node* cursor, Node* newNode, int level) {
			...
			while (
				cursor->next[level]
				&& 1 == (phi = Compare(newNode->payload, cursor->next[level]->payload))
				)
		...
		static Node* remove(Node* cursor, const T& p, int level) {
			...
			while (
				cursor->next[level]
				&& 1 == (phi = Compare(p, cursor->next[level]->payload))
				)
				cursor = cursor->next[level];
	...
	public:
		...
		bool push(const T& payload) {
			Node* n = new Node(payload, next_random(_levels) + 1);
